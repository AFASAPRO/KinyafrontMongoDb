<template>
  <div class="admin-auth">
    ...
    <div class="ab-card">
      ...
      <transition name="form-slide" mode="out-in">
        <div v-if="mode === 'login'" key="login" class="ab-form">
          ...
          <div class="ab-hint">
            ...
            <div>
              Default: <code>admin@kinyabot.ai</code> /
              <code>KinyaBot@Admin2024</code>
              <span class="ab-hint-note">Run <code>node setup-admin.js</code> first</span>
            </div>
          </div>
        </div>
      </transition>
      ...
      <!-- ── Register Form ── -->
      <transition name="form-slide" mode="out-in">
        <div v-if="mode === 'register'" key="register" class="ab-form">
          <!-- Username -->
          <div class="ab-field" :class="{ focused: focus.rUsername, hasVal: regForm.username }">
            ...
            <input
              ...
              @focus="focus.rUsername = true"
              @blur="focus.rUsername = false"
            />
          </div>
          <span v-if="regErrors.username" class="ab-err">
            <i class="fas fa-circle-exclamation"></i> {{ regErrors.username }}
          </span>

          <!-- Email -->
          <div class="ab-field" :class="{ focused: focus.rEmail, hasVal: regForm.email }">
            <div class="ab-field-icon"><i class="fas fa-envelope"></i></div>
            <input
              ...
              class="ab-input"
              autocomplete="email"
              @focus="focus.rEmail = true"
              @blur="focus.rEmail = false"
            />
          </div>
          <span v-if="regErrors.email" class="ab-err">
            <i class="fas fa-circle-exclamation"></i> {{ regErrors.email }}
          </span>

          <!-- Password -->
          <div class="ab-field" :class="{ focused: focus.rPassword, hasVal: regForm.password }">
            <div class="ab-field-icon"><i class="fas fa-lock"></i></div>
            <input
              ...
              @focus="focus.rPassword = true"
              @blur="focus.rPassword = false"
            />
            <button type="button" class="ab-eye" @click="showRegPass = !showRegPass">
          ...
          <span v-if="regErrors.password" class="ab-err">
            <i class="fas fa-circle-exclamation"></i> {{ regErrors.password }}
          </span>

          <!-- Confirm password -->
          <div class="ab-field" :class="{ focused: focus.rConfirm, hasVal: regForm.confirm, 'ab-field-error': regForm.confirm && regForm.confirm !== regForm.password }">
            <div class="ab-field-icon"><i class="fas fa-lock-open"></i></div>
            <input
              ...
              class="ab-input"
              autocomplete="new-password"
              @focus="focus.rConfirm = true"
              @blur="focus.rConfirm = false"
            />
            <div v-if="regForm.confirm && regForm.confirm === regForm.password" class="ab-match-icon">
              <i class="fas fa-check" style="color:#34d399"></i>
            </div>
          </div>
          <span v-if="regForm.confirm && regForm.confirm !== regForm.password" class="ab-err">
            <i class="fas fa-circle-exclamation"></i> Passwords don't match
          </span>

          <!-- Role selector -->
          <div class="ab-field" :class="{ focused: focus.rRole }">
            <div class="ab-field-icon"><i class="fas fa-user-tie"></i></div>
            <select
              v-model="regForm.role"
              class="ab-input ab-select"
              @focus="focus.rRole = true"
              @blur="focus.rRole = false"
            >
          ...
          <!-- Invite code -->
          <div class="ab-field" :class="{ focused: focus.rInvite, hasVal: regForm.invite_code }">
            ...
            <input
              ...
              class="ab-input"
              @focus="focus.rInvite = true"
              @blur="focus.rInvite = false"
            />
            <button type="button" class="ab-eye" title="Use default code" @click="regForm.invite_code = 'KinyaBot-Admin-2024'">
              <i class="fas fa-wand-magic-sparkles" style="font-size:12px"></i>
            </button>
          </div>
          <span v-if="regErrors.invite_code" class="ab-err">
            <i class="fas fa-circle-exclamation"></i> {{ regErrors.invite_code }}
          </span>

          <div v-if="regError" class="ab-server-err">
            <i class="fas fa-circle-exclamation"></i> {{ regError }}
          </div>

          <button class="ab-btn" :disabled="regLoading || !isRegFormValid" @click="handleRegister">
            <span v-if="regLoading" class="ab-btn-inner">
          ...
          <div class="ab-invite-hint">
            ...
            <div>
              ...
              <code
                ...
                @click="regForm.invite_code = 'KinyaBot-Admin-2024'"
              >KinyaBot-Admin-2024</code>
              <span class="ab-hint-note">Click the code above to auto-fill, or set <code>ADMIN_INVITE_CODE</code> in backend <code>.env</code></span>
            </div>
          </div>
        </div>
      </transition>
      ...
      <transition name="fade">
        <div v-if="registered" class="ab-success">
          ...
          <p>Welcome, <strong>{{ registeredName }}</strong>. You are now signed in.</p>
